...
def noise_p(X, n):
    ...
    # Calculate the proportion of pixels to be noised per image
    ratio = tf.cast(n, tf.float32) / tf.cast(total_pixels, tf.float32)

    # Generate random noise mask in [0,1) for each pixel
    rand_mask = tf.random.uniform((a, b, c), dtype=tf.float32)

    # True means this pixel should be noised
    noise_mask = rand_mask < ratio  # shape = (a,b,c)

    # Generate random 0/1 values for selected pixels
    ...
    # Direct replacement using tf.where (GPU parallel)
    X_p = tf.where(noise_mask, noise_values, X)
    return X_p

def noise_m(X, degree, padding_mode="REFLECT"):
    """
    GPU-accelerated random motion blur (no black border version)
    Args:
        X: [N, H, W, C] Tensor, range 0~1 or 0~255
        degree: blur kernel size
        padding_mode: edge padding mode, options {"REFLECT", "SYMMETRIC", "REPLICATE"}
    """
...
def noise_v(X, v, random_color=False):
    """
    GPU-accelerated version: Add random rectangular noise blocks with fixed area to input batch images.

    Args:
        X: Input image tensor, shape (N, H, W, C), range [0,1]
        v: Target rectangle area (number of pixels)
        random_color: If True, use random color blocks; otherwise use white blocks (1.0)

    Returns:
        X_v: Image with rectangular noise added, same shape as X
    """
    X = tf.convert_to_tensor(X, dtype=tf.float32)
    N, H, W, C = X.shape

    # --- Random aspect ratio, but keep area fixed ---
    ...
    # --- Randomly generate top-left corner of rectangle ---
...
class BaseClassificationAdversaryL2:
    """
    Base class for adversarial attacks on multi-class classification models (L2 constraint, undirected: maximize Cross-Entropy)
    Input: images [B,H,W,C]
    Compatible y formats:
    ...
    def __init__(self, model, num_classes=4, clip_min=0.0, clip_max=1.0, seed=None,
        ...
        # Auto infer logits/prob: if user doesn't specify from_logits, check the last layer's activation
        if from_logits is None:
            ...
            try:
                ...
                act = None
            # softmax => probs; others/unknown => treat as logits (more stable)
            self.from_logits = False if (act is not None and act.__name__ == "softmax") else True
        else:
            self.from_logits = bool(from_logits)

        # Cross-entropy object (reduction=None for per-sample access)
    ...
    def _to_onehot(self, y):
        ...
        if y.shape.rank == 1:
            ...
            # If dimension mismatch, it will be exposed during training
            return y
    ...
    def loss_mean(self, x, y):
        """
        Batch-averaged CE (scalar), used for PGD gradient ascent
        """
    ...
    def generate_adversarial_dataset(
        ...
        """
        Automatically generate adversarial samples by calling self.attack_batch in batches and concatenate results.
        Returns: x_adv_all
        """
...
class PGDClassificationAdversaryL2(BaseClassificationAdversaryL2):
    """
    PGD white-box attack (L2 constraint, undirected: maximize Cross-Entropy)
    - Same interface as PGDRegressionAdversaryL2 for seamless replacement
    """
    ...
    def attack_batch(
        ...
        """
        Generate adversarial samples for a batch (white-box: 1 forward + 1 backward per step)
        xb: [B,H,W,C]
...
class BaseRegressionAdversaryL2:
    """
    Base class for adversarial attacks on single-output regression models (L2 constraint, undirected: maximize MSE)
    Input: images [B, 32, 32, C] or general [B, H, W, C]
    """
    ...
    def _mse_per_sample(y_pred, y_true):
        """Scalar regression per-sample MSE: returns [B]"""
        y_pred = tf.reshape(y_pred, [-1])
        y_true = tf.reshape(y_true, [-1])
        return tf.square(y_pred - y_true)

    def loss_mean(self, x, y):
        """Batch-averaged MSE (scalar)"""
        pred = self.model(x, training=False)
    ...
    def generate_adversarial_dataset(
        ...
        **attack_kwargs,
        ...
        """
        Input entire dataset (x_all, y_all), automatically generate adversarial samples by calling self.attack_batch in batches and concatenate results.
        Subclasses must implement: attack_batch(xb, yb, **attack_kwargs) -> xb_adv

        Returns:
          x_adv_all, y_out (y_out matches y_all)
        """
        x_all = tf.convert_to_tensor(x_all, dtype=tf.float32)
...
class PGDRegressionAdversaryL2(BaseRegressionAdversaryL2):
    """
    PGD white-box attack (L2 constraint, undirected: maximize MSE)
    """
    ...
    def attack_batch(
        ...
        """
        Generate adversarial samples for a batch
        """
        x0 = tf.cast(xb, tf.float32)
