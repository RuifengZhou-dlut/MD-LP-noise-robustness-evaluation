...
class CustomWeightDecaySGD(tf.keras.optimizers.SGD):
    ...
    def get_config(self):
        ...
        config.update({
            "weight_decay": float(self.weight_decay),  # ensure float type
        })
        return config
...
class LastNSaver(tf.keras.callbacks.Callback):
    def __init__(self, n=10):
        super().__init__()
        self.n = n
        self.history = deque(maxlen=n)  # store last N (val_acc, weights) pairs
    ...
    def on_train_end(self, logs=None):
        # select best from last N epochs
        if not self.history:
            return
...
def mixup_batch(x, y, alpha=0.2):
    ...
    # Sample from Beta(alpha, alpha): implemented using Gamma approximation to avoid dependency on tfd
    ...
    lam_y = tf.reshape(lam[:, 0, 0, 0], [batch_size, 1])
    y = y * lam_y + y2 * (1.0 - lam_y)
    return x, y

def randaugment_mild(x, y, strong_aug=False):
    # mild color jitter (skip hue/saturation for now)
